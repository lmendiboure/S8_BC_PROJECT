...
export class DiffusionComponent implements OnInit {
  ...
  onFileSelected(event) { //permet de choisir le nom de la vidéo à envoyé
    if (event.target.files.length > 0) {
      ...
      for (let idx in this.usersGroup) {
        // prend la liste des noms de groupes auquel on fait parti
        //ainsi que la liste des ip des groupes
        this.grIP.push(this.usersGroup[idx].grIP);
        this.grName.push(this.usersGroup[idx].name);
      }
    }
  }

  onGroupSelection(groupSelection){
    //permet de choisir auquel envoyé pour la diffusion il reste à faire la requête get
    //sur le server regarder plus bas comment faire la requête, remplace le port et le chemin et demander à youssef
    // si il faut lui passer le nom de group en paramètre ou dans body
    console.log(groupSelection);
    this.groupeSelec=groupSelection;
    //faire requête mongoDB pour avoir correspondance ip nom de groupe
    for(let idx in this.grName){
      console.log("on est dedans");
      if (this.groupeSelec.localeCompare(this.grName[idx] )==0) {
        this.group=this.grIP[idx]; //correspondance IP/ nom de groupe
      }
    }
  ...
  ngOnClickDiffusion(){
    //est appelée quand le bouton envoyé la vidéo est appuyé,
    //normalement rien a changé, il faut que le path dans le send de la fonction get de diffusion
    //du serv_local corresponde au dossier où la vidéo a été trouvé, car this.video ne récupère que le nom de la vidéo,
    // essayé de récupérer le path à la place de vidéo pour améliorer.
